...
//	{
//		std::vector<std::string>	postfix = toPostfix(move(expression));
//		std::vector<StatData>		Vars;
//
//		for (auto it = postfix.begin(), end = postfix.end(); it != end; ++it)
//		{
//			unsigned operation = strToOp(*it);
//
//			if (operation == 0)
//			{
//				long long number = 0;
...
//	};
//
//	virtual unsigned strToOp(const char c, const char cNext, unsigned* i = nullptr) = 0;
//
//	virtual std::string opToStr(unsigned operation) noexcept = 0;
//
//	virtual unsigned priority(unsigned operation) = 0;
//
//	virtual bool process(std::string operationName, StatData *data, StatData *data2, StatData &result, std::string &expression) = 0;
//
//	unsigned strToOp(std::string oper)
//	{
//		if (oper.empty() || oper.size() > 2) return 0;
//		return strToOp(oper[0], ((oper.size() == 1) ? '\0' : oper[1]));
//	}
//
//	unsigned priority(std::string elem)
//	{
//		for (char c : elem)
...
//				unsigned operation = strToOp(c, cNext, &i);
//				if (!operation)
...
//	unsigned strToOp(const char c, const char cNext, unsigned *i = nullptr)
//	{
//		Operation operation;
//		switch (c)
//		{
...
//	}
//
//	std::string opToStr(unsigned operationID) noexcept
//	{
...
//	}
//
//	unsigned priority(unsigned operationID)
//	{
//		Operation operation = static_cast<Operation>(operationID);
//		switch (operation)
//		{
...
//	};
//
//	static unsigned strToOp(const char c, const char cNext, unsigned* i = nullptr)
//	{
//		Operation operation;
//		switch (c)
//		{
...
//	}
//
//	static std::string opToStr(unsigned operationID) noexcept
//	{
//		Operation operation = static_cast<Operation>(operationID);
//		switch (operation)
//		{
