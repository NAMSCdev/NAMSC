...
public:
	ImageAsset() = default;
	ImageAsset(QString &&name, QString &&location, unsigned pos = 0) :
		Asset(move(name), move(location), pos) {}
	ImageAsset(const ImageAsset& asset)			  { *this = asset; }
	ImageAsset& operator=(const ImageAsset& asset){ Asset::operator=(asset); }
