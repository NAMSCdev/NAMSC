...
public:
	...
	ActionFilterBlur(QString &&label, QString &&sceneryObjectName, double intensivness, 
					 double strength, BlurType blurType) :
		ActionFilter(move(label), move(sceneryObjectName), intensivness, strength), blurType(blurType) {}
...
signals:
	...
	void onRun(SceneryObject *sceneryObject, QPoint pos, QSize size, double intensivness, uint strength, BlurType blurType);
