...
public:
	...
	ActionEffectDistort(QString &&label, QString &&sceneryObjectName, EffectShape effectShape, QPoint pos,
						QSize size, uint strength) :
		ActionEffect(move(label), move(sceneryObjectName), effectShape, pos, size, strength) {}
...
signals:
	...
	void onRun(SceneryObject *sceneryObject, EffectShape effectShape, QPoint pos, QSize size, uint strength);
